...
export default function Home(){
  ...
  return <main>
    ...
    <section className="hero no-visual" id="inicio"><div className="hero-copy"><div className="heading-row"><p className="eyebrow">TU PRÓXIMO VEHÍCULO ESTÁ AQUÍ</p><button className="edit-link" onClick={()=>setEditing(!editing)}>{editing?'Listo':'✎ Editar encabezado'}</button></div>{editing?<textarea className="heading-editor" value={heading} onChange={e=>setHeading(e.target.value)} aria-label="Editar encabezado"/>:<h1>{heading.split(' siempre ')[0]}<br/><em>{heading.includes(' siempre ')?'siempre '+heading.split(' siempre ')[1]:''}</em></h1>}<p className="hero-text">Conoce este vehículo, descubre cada característica y encuentra un financiamiento flexible con una garantía que sí responde.</p><div className="hero-actions"><a href="#vehiculo" className="text-link">Conocer el vehículo <span>↓</span></a></div><div className="trust"><div><strong>60</strong><span>meses de<br/>financiamiento</span></div><div><strong>20%</strong><span>de enganche<br/>desde</span></div><div><strong>✓</strong><span>GARANTÍA EL<br/>TANQUE MOTORS</span></div></div></div></section>
