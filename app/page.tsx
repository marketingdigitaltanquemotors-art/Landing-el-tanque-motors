...
type Vehicle = { name:string; year:string; km:string; price:number; features:string; video?:string; images?:string[] };
...
export default function Home(){
  ...
  function uploadImages(index:number,e:ChangeEvent<HTMLInputElement>){const files=Array.from(e.target.files??[]);if(!files.length)return;const images=files.map(file=>URL.createObjectURL(file));setVehicles(list=>list.map((v,i)=>i===index?{...v,images:[...(v.images??[]),...images]}:v));}
  ...
  return <main>
    ...
    <section className="hero no-visual" id="inicio"><div className="hero-copy"><div className="heading-row"><p className="eyebrow">TU PRÓXIMO VEHÍCULO ESTÁ AQUÍ</p><button className="edit-link" onClick={()=>setEditing(!editing)}>{editing?'Listo':'✎ Editar encabezado'}</button></div>{editing?<textarea className="heading-editor" value={heading} onChange={e=>setHeading(e.target.value)} aria-label="Editar encabezado"/>:<h1>{heading.split(' siempre ')[0]}<br/><em>{heading.includes(' siempre ')?'siempre '+heading.split(' siempre ')[1]:''}</em></h1>}<p className="hero-text">Conoce este vehículo, descubre cada característica y encuentra un financiamiento flexible con una garantía que sí responde.</p></div></section>

    <section className="inventory single-vehicle" id="vehiculo"><div className="inventory-top"><div><p className="eyebrow">CONOCE EL VEHÍCULO</p><h2>Hecho para ti.</h2></div><div><p>Muestra su video y edita las características para que tu cliente conozca cada detalle.</p></div></div><div className="vehicle-grid">{vehicles.map((v,i)=><article className="vehicle-tile" key={`${v.name}-${i}`}><div className="video-manager"><div className="tile-media">{v.video?<video src={v.video} controls playsInline/>:<div className="mini-car"><div/></div>}<label className="upload-btn"><input type="file" accept="video/*" onChange={e=>upload(i,e)}/>{v.video?'Cambiar video':'＋ Subir video'}</label></div></div><section className="image-manager"><div className="image-manager-heading"><div><p className="eyebrow">GALERÍA DEL VEHÍCULO</p><h3>Fotos desde todos los ángulos</h3></div><label className="btn image-upload"><input type="file" accept="image/*" multiple onChange={e=>uploadImages(i,e)}/>＋ Subir imágenes</label></div>{v.images?.length?<div className="angle-gallery">{v.images.map((src,n)=><figure key={src}><img src={src} alt={`${v.name}, ángulo ${n+1}`}/><figcaption>Ángulo {n+1}</figcaption></figure>)}</div>:<div className="image-empty">Sube varias imágenes del vehículo para mostrar el frente, los costados, la parte trasera y el interior.</div>}</section><div className="tile-info detached-info"><p>{v.year} · AUTOMÁTICO</p><input className="vehicle-name" value={v.name} onChange={e=>setVehicles(list=>list.map((x,n)=>n===i?{...x,name:e.target.value}:x))} aria-label="Nombre del vehículo"/><div className="specs"><span>{v.km}</span><span>Gasolina</span></div><label className="features-label" htmlFor={`features-${i}`}>Características del vehículo</label><textarea id={`features-${i}`} className="features-editor" value={v.features} onChange={e=>setVehicles(list=>list.map((x,n)=>n===i?{...x,features:e.target.value}:x))} aria-label={`Características de ${v.name}`}/><div className="vehicle-price"><span>Precio</span><strong>{money(v.price)}</strong></div><button className="btn vehicle-quote" onClick={()=>quote(v)}>Cotiza aquí mismo <span>↗</span></button></div></article>)}</div><div className="trust vehicle-trust"><div><strong>60</strong><span>meses de<br/>financiamiento</span></div><div><strong>20%</strong><span>de enganche<br/>desde</span></div><div><strong>✓</strong><span>GARANTÍA EL<br/>TANQUE MOTORS</span></div></div></section>
